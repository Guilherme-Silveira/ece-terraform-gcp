...
# ECE version
CLOUD_ENTERPRISE_VERSION=2.7.1
  ...
  while [ "$1" != "" ]; do
    case $1 in
      ...
      --help|help)
                        ...
                        echo "[--roles-token TOKEN] [--roles \"ROLES\"] [--force] [--api-base-url API_BASE_URL]"
                        echo ""
                        echo "Arguments:"
...
parseResetAdminconsolePasswordArguments() {
  ...
  if [[ -z "$BOOTSTRAP_SECRETS" ]]; then
      ...
      ZK_ROOT_PASSWORD=$(docker -H "unix://${HOST_DOCKER_HOST}" exec frc-directors-director bash -c 'echo -n $FOUND_ZK_READWRITE' 2>/dev/null | cut -d: -f 2)
...
parseUpgradeArguments() {
  ...
  SOURCE_CONTAINER_NAME="frc-runners-runner"
  HOST_STORAGE_PATH=$(docker -H "unix://${HOST_DOCKER_HOST}" exec $SOURCE_CONTAINER_NAME bash -c 'echo -n $HOST_STORAGE_PATH' 2>/dev/null | cut -d: -f 2)
  ...
  ZK_ROOT_PASSWORD=$(docker -H "unix://${HOST_DOCKER_HOST}" exec $SOURCE_CONTAINER_NAME bash -c 'echo -n $FOUND_ZK_READWRITE' 2>/dev/null | cut -d: -f 2)
  if [[ -z "${ZK_ROOT_PASSWORD}" ]]; then
